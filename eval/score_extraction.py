...
def npt(s): return re.sub(r"^(P|PT|PATIENT|SUBJECT|CASE)[\s._-]*", "", (s or "").strip(), flags=re.I)

# ----- load -------------------------------------------------------------------
GOLD_PATH, AGENT_PATH = sys.argv[1], sys.argv[2]
report_path = None; allow_unverified = False
for i, a in enumerate(sys.argv[3:]):
    if a == "--report": report_path = sys.argv[4 + i]
...
def imp_identity(i): return (npt(i.get("patient_paper_id", "")), (i.get("gene_symbol") or "").upper(), nmut(i.get("mutation")))

agent_all_neo = {imp_identity(i) for i in imp.values()}
...
for e in agent.get("evidence", []):
    if e.get("outcome") in ("immunogenic", "positive"):
        for i in evidence_to_imps(e):
            ident = imp_identity(i); agent_immuno_neo.add(ident)
...
# ----- gold-side sets ---------------------------------------------------------
gold_all_neo = {(npt(n["patient"]), n["gene"].upper(), nmut(n.get("mutation"))) for n in gold.get("neoantigens", [])}
gold_immuno_neo = {(npt(n["patient"]), n["gene"].upper(), nmut(n.get("mutation"))) for n in gold.get("neoantigens", []) if n.get("immunogenic")}
gold_epi = {(nseq(e["sequence"]), e["mhc_class"]) for e in gold.get("epitopes", [])}
gold_mag = {(npt(m["patient"]), (m.get("gene") or "").upper(), nmut(m.get("mutation"))): m for m in gold.get("magnitudes", [])}
...
ok_to_use = gold.get("verified") or allow_unverified
